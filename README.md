...
&emsp;エネルギーの計算が完了すれば終了です。せっかくなので、エネルギーバンド図を作成しましょう。そのためのプログラムをコピーしましょう。
...
&emsp;最後に、 __先ほどのLAMMPSの一点計算の結果と比較しましょう。__ 「nohup.out」の下の方にエネルギーの計算結果が出力されています。  
...
時間経過で nohup.out に結果が記録されていくので、適宜確認しましょう。50タイムステップ分が実行されたら完了です。時間がかかるので、INCARタグの意味を調べたり、次のチュートリアルに進んでください。
<br>  
