...
&emsp;本チュートリアルは __6.3PC__ で行いましょう。詳しい性能差は、Boxの「計算班PCリスト.xlsx」を参照してください。  
&emsp;ネットワーク接続手順のおさらいですが、Teratermで __6.2PC__ にログインしましょう。ログイン出来たら、次のコマンドを実行し、他のPCに移動しましょう。[ ]は不要です。

     ssh -X -Y owner@192.168.6.3
...
今回のチュートリアルでは、NaClの計算を行いたいので、構造ファイルを __Materials Project__ から持ってきましょう。  

     https://next-gen.materialsproject.org/materials
     
サイト内の __Materials Explorer__ より __NaCl__ と検索すると、__mp-数字__ が複数出てくると思います。その中で __Crystal style__ が適切な物を1つ選びましょう。複数存在する場合は★マークがついているものや数字が少ないものを選ぶと良いでしょう。

選んだら、構造が表示されると思いますが、その右上にある4つのアイコンの一番下の __Export as__ を選択し、 __POSCAR__ を選択しましょう。
